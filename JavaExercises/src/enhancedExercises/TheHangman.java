		...
		try {
			...
			do { // Bucle con interfaz para el usurario
				...
				case 'a': // Comienza el juego!
					word = Methods.chooseWord(word); // Se escoje una plabra aleatoria del metodo creado en la clase
														// "Methods".
					int x = 0;
					System.out.println(
					...
					System.out.println(Arrays.toString(word_2)); // Se imprimen las letras en el terminal para facilitar
																	// la correcion corregir. ;)

					char word_3[] = new char[x]; // Array donde almacenara los espacios vacios equivalentes a cada letra
													// de la palabra.
					for (int i = 0; i < word_3.length; i++) {
						word_3[i] = '_';
					}

					for (int i = 6; i > 0; i--) { // Bucle que se ejecutara hasta que no se acaben las vidas, o hasta
													// que se rompa con un condicional que esta mas abajo.
						char attempts = JOptionPane.showInputDialog(null,
								Arrays.toString(word_3) + "\nIntroduce una letra:" + "\nTe quedan " + i + " vidas.\n")
								.toUpperCase().charAt(0);

						for (int j = 0; j < word.length(); j++) { // Si la letra introducida por el usuario pertenece al
																	// Array 2 con las letras correspondientes a
							if (word_2[j] == attempts) { // la palabra escogida se sobre escribe el Array 3
								word_3[j] = attempts;
							}
						}

						for (int j = 0; j < word.length(); j++) { // Si el usuario acierta, se mantiene la misma
																	// cantidad de vidas
						...
						if (Arrays.equals(word_2, word_3)) { // Se comprueba en cada intento si el Array 2 y el Array 3
																// son iguales, y de cumplirse la condicion
					...
					if (Arrays.equals(word_2, word_3) == false) { // Al acabarse todas las vidas y salir del bucle
						JOptionPane.showMessageDialog(null, // comprueba que ambos Arrays son distintos, y anuncia que
															// el usuario perdio.
								"Palabra: " + word + "\nHas perdido." + "\n¡Hasta pronto!");
					}
					break;
				...
				default:
					JOptionPane.showMessageDialog(null, "El caracter introducido no es valido, intentalo de nuevo."); // Mensaje
																														// si
																														// el
																														// usuario
																														// introduce
